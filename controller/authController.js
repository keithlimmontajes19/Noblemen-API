  ...
  try {
    ...
    if (data) {
      ...
      if (data.isVerified) {
        ...
        bcryptjs.compare(password, data.password, (err, result) => {
          ...
          if (result) {
            ...
            return res
              .status(200)
              .json(RESPONSE(200, CONSTANTS.LOGIN_SUCCESS, token));
          } else {
            return res
              .status(400)
              .json(
      ...
      return res
        .status(400)
        .json(
          RESPONSE(400, CONSTANTS.NO_RESULT, {}, STATUS_CODE.NO_RESULT_FOUND),
        );
    }
    /* end checker email */
  } catch (err) {
    return res
      .status(400)
      .json(
        RESPONSE(400, CONSTANTS.NO_RESULT, err, STATUS_CODE.NO_RESULT_FOUND),
      );
  }
};
...
export const userTokenVerification = async (user, req, res) => {
  try {
    ...
    return res
      .status(500)
      .json(
        RESPONSE(500, CONSTANTS.USER_FAILED, err, STATUS_CODE.ADD_USER_FAILED),
      );
  }
};
...
export const confirmationRegister = async (req, res, next) => {
  try {
    ...
    return res.redirect(`${process.env.REACT_BASE_URL}?vertification=false`);
  }
};
